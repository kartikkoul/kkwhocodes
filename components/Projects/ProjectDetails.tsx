    ...
    <AnimatePresence mode="wait">
      {visible && project ? (
        ...
        >
          ...
          <h3
            className="font-montserrat text-2xl font-light sm:text-[1.75rem]"
            style={{ color: project.accent }}
          >
            {project.title}
          </h3>
          <p className="mt-1 font-mono text-sm text-white/55">{project.tagline}</p>
          <p className="mt-4 font-poppins text-sm leading-relaxed text-white/70">
            {project.description}
          </p>
          ...
          <motion.div className="mt-6" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            <Link
              ...
              className="inline-flex items-center gap-2 rounded border-2 px-5 py-2.5 font-poppins text-sm font-medium text-white transition-shadow duration-300"
              style={{
